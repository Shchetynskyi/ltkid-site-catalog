...
## UI v1.2 – Card Hierarchy (Gallery + Model)

Дата: 2026-02-16

Прийнято:

1. Ієрархія відображення:
   - Ціна (головний акцент)
   - Маркетингова назва (якщо є)
   - Тип лінз (людський текст)
   - ModelID + виробничий тег (ФХС / тон.)

2. Якщо marketingTitle відсутня:
   - ModelID стає головною назвою (id-primary)

3. TypeLens:
   - NONE → не показується
   - BB → Захист від шкідливого впливу екрана
   - PHOTO → Фотохромні (хамелеони)
   - TINT → Тонові лінзи
...
## UI-HEADER-1: Глобальна навігація + локальний контроль галереї

**Дата:** 2026-04-14  
**Статус:** Прийнято

### Контекст
На мобільному у галереї було 3–4 рядки зверху:
- окремий рядок "Початок"
- sticky toolbar з фільтрами
- рядок з "Показано: N" і перемикачем колонок

Це створювало перевантаження і зменшувало корисну висоту екрану.

### Рішення

1. **Глобальна навігація**
- `Початок` залишається в `src/routes/+layout.svelte`
- реалізовано як `sticky` header
- доступний на всіх екранах, крім головного (`/`)

2. **Локальний контроль галереї**
- перемикач `1 / 2 колонки` винесено в праву частину header
- показується тільки в галереї через store:
  - `showGalleryViewToggle`
  - `gallerySingleColumn`

3. **Спрощення UI**
- лічильник `Показано: N` повністю видалено
- дубль кнопки перемикання колонок у галереї видалено

### Наслідки

**Плюси:**
- мінус 1 рядок UI зверху
- чітке розділення:
  - глобальна навігація (header)
  - контент (фільтри)
- більше простору для контенту на мобільному

**Мінуси:**
- header отримує умовний елемент (контекст-залежний UI)

### Альтернативи (відхилені)

- Залишити перемикач в галереї  
  → зайвий рядок

- Об’єднати "Початок" і фільтри  
  → змішування навігації і контенту

- Прибрати фільтри  
  → порушення основного UX