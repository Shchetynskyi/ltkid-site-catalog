...
він відображає те, що бачив клієнт на сайті, і дає менеджеру контекст.

### Canonical LeadPayload (завжди на Model Page)
Передається при кліку CTA “Звʼязатися з менеджером” на сторінці моделі:

- `ModelID`
- `MarketingTitle` — маркетингова назва
- `SitePriceUAH` — ціна, яку клієнт бачив у UI  
  (якщо в UI було “Ціну уточнюйте” — передається саме цей текст)
- `Image` — 1 зображення (preview або main image з сайту)
- `ref` — джерело сценарію:
  - `site_catalog__model`
  - `site_catalog__frame`
...
- “вгадувати” діоптрію або додавати діоптрії без дії користувача
...
ModelID не відображається в Hero Model Page; зберігається в даних і LeadPayload.
